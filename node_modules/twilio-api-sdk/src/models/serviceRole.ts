/**
 * Twilio APIsLib
 *
 * This file was automatically generated by APIMATIC v3.0 ( https://www.apimatic.io ).
 */

import {
  array,
  nullable,
  optional,
  Schema,
  string,
  typedExpandoObject,
  unknown,
} from '../schema.js';
import {
  ServiceRoleRoleType,
  serviceRoleRoleTypeSchema,
} from './serviceRoleRoleType.js';

export interface ServiceRole {
  /** The unique string that we created to identify the Role resource. */
  sid?: string | null;
  /** The SID of the [Account](https://www.twilio.com/docs/iam/api/account) that created the Role resource. */
  accountSid?: string | null;
  /** The SID of the [Conversation Service](https://www.twilio.com/docs/conversations/api/service-resource) the Role resource is associated with. */
  chatServiceSid?: string | null;
  /** The string that you assigned to describe the resource. */
  friendlyName?: string | null;
  /** The type of role. Can be: `conversation` for [Conversation](https://www.twilio.com/docs/conversations/api/conversation-resource) roles or `service` for [Conversation Service](https://www.twilio.com/docs/conversations/api/service-resource) roles. */
  type?: ServiceRoleRoleType;
  /** An array of the permissions the role has been granted. */
  permissions?: string[] | null;
  /** The date and time in GMT when the resource was created specified in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format. */
  dateCreated?: string | null;
  /** The date and time in GMT when the resource was last updated specified in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format. */
  dateUpdated?: string | null;
  /** An absolute API resource URL for this user role. */
  url?: string | null;
  additionalProperties?: Record<string, unknown>;
}

export const serviceRoleSchema: Schema<ServiceRole> = typedExpandoObject(
  {
    sid: ['sid', optional(nullable(string()))],
    accountSid: ['account_sid', optional(nullable(string()))],
    chatServiceSid: ['chat_service_sid', optional(nullable(string()))],
    friendlyName: ['friendly_name', optional(nullable(string()))],
    type: ['type', optional(serviceRoleRoleTypeSchema)],
    permissions: ['permissions', optional(nullable(array(string())))],
    dateCreated: ['date_created', optional(nullable(string()))],
    dateUpdated: ['date_updated', optional(nullable(string()))],
    url: ['url', optional(nullable(string()))],
  },
  'additionalProperties',
  optional(unknown())
);
